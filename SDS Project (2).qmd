...
-   This dataset is particularly interesting to us because it relates to our major. We are Human Development and Family Science majors, which allows us to explore the relationships between various maternal and paternal factors, such as gestation periods. Understanding these relationships can provide insights into factors that influence pregnancy outcomes, which is valuable for public health and medical research.
...
ggplot(Gestation_edits, aes(x = dht, y = gestation)) + 
  geom_point() +
  geom_smooth(method = "lm", formula = y ~ x, color = "lavender") +
  labs(
...
exponentiated_coefficients <- exp(c(5.2786621, 0.2239463, 0.0030834, 0.0028263, 0.0001022, 0.0884276, 0.0011319, 0.0040986))
names(exponentiated_coefficients) <- c("Intercept", "Weight", "Smoke Now", "Smoke Did, Not Now", "Smoke Until Current Pregnancy", "Age", "Parity", "Paternal Height")
